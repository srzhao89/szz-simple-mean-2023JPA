...
#### For VRS and p+q>=4 ####
coverage.vrs2 <- function(x,y,L=20) {
  ...
  d0= 1/FEAR::dea(XOBS=x,YOBS=y,XREF=x,YREF=y, 
                  RTS=1,ORIENTATION=2)
  ...
  # tbar/tabr0 contains the bias for eff of each obs
  ...
  var.bc.sz=var+mu.bias^2 # sz2020 correction method
  #
  ...
  bounds4=matrix((muk-mu.bias-ts4*crit),nrow=3,ncol=2)
  #
  ...
  bounds40=matrix((muk0-mu.bias0-ts40*crit),nrow=3,ncol=2)
  #
  ...
  res=list(bounds1=bounds1,bounds2=bounds2,
           ...
           estimate=c(muk,mu.bias,muk-mu.bias,lambda.s,lambda.t),
           estimate0=c(muk0,mu.bias0,muk0-mu.bias0,lambda.s,lambda.t)
  )
  return(res)
}
